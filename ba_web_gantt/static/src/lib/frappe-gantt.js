...
class Bar {
    ...
    draw_bar() {
        this.$bar = createSVG('rect', {
            ...
            class: this.task.invalid ? 'bar' : 'bar oe_gantt_color_' + this.task.color,
    ...
    draw_progress_bar() {
        ...
        var clss = 'bar-progress';
        if (this.progress_width > this.width) var clss = 'bar-progress-alert';
    ...
    draw_resize_handles() {
        ...
        createSVG('rect', {
            ...
            rx: this.corner_radius,
            ry: this.corner_radius,
            class: this.task.invalid_end ? 'handle right handle-invalid' : 'handle right',
        ...
        createSVG('rect', {
            ...
            rx: this.corner_radius,
            ry: this.corner_radius,
            class: this.task.invalid_start ? 'handle left handle-invalid' : 'handle left',
            append_to: this.handle_group
        });
...
*/        
    ...
    show_popup() {
        const start_date = date_utils.format(this.task._start, 'D MMM HH:mm');
        const end_date = date_utils.format(this.task._end, 'D MMM HH:mm');
      
        const dur = date_utils.diff(this.task._end, this.task._start, 'hour');
        const days = Math.floor(dur / 24);
        const hours = dur - days * 24;
        var prgrs = '';
        var user_id = '';
        if (this.task.progress) {
            prgrs += '<br/>progress: ' + this.task.progress + ' %';
        };
        if (this.task.user_id) {
            user_id += '<br/>assigned: ' + this.task.user_id[1];
        };
      
        const subtitle = 'start: ' + start_date + '<br/>stop: ' + end_date + 
              '<br/>duration: ' + days + ' day(s) ' + hours + ' hour(s)' + prgrs + user_id;
...
class Gantt {
    ...
    setup_options(options) {
        const default_options = {
            ...
            column_width: 30,
            step: 24,
            view_modes: ['Hour', 'Two Hour', 'Quarter Day', 'Half Day', 'Day', 'Week', 'Month'],
            bar_height: 20,
            bar_corner_radius: 3,
            arrow_curve: 5,
            padding: 18,
    ...
    setup_tasks(tasks) {
        ...
        this.tasks = tasks.map((task, i) => {
            ...
            // invalid flag
            task.invalid = false;
            task.invalid_start = false;
            task.invalid_end = false
            if (!task.start || !task.end) {
                task.invalid = true;
            }
            if (!task.start) {
                task.invalid_start = true;
            }
            if (!task.end) {
                task.invalid_end = true;
            }
    ...
    setup_gantt_dates() {
        ...
        } else if (this.view_is('Two Hour')) {
            this.gantt_start = date_utils.add(this.gantt_start, -3, 'day');
            this.gantt_end = date_utils.add(this.gantt_end, 3, 'day');
        } else if (this.view_is('Quarter Day')) {
            this.gantt_start = date_utils.add(this.gantt_start, -7, 'day');
            this.gantt_end = date_utils.add(this.gantt_end, 7, 'day');
        } else if (this.view_is('Half Day')) {
            this.gantt_start = date_utils.add(this.gantt_start, -12, 'day');
            this.gantt_end = date_utils.add(this.gantt_end, 12, 'day');
    ...
    make_grid_highlights() {
            const x = Math.floor(
                date_utils.diff(new Date(), this.gantt_start, 'hour') /
                this.options.step) *
                this.options.column_width;
            const y = 0;
    ...
    make_dates() {
        for (let date of this.get_dates_to_draw()) {
            ...
            //for Firefox  
            try {
                if ($upper_text.getBBox().x2 > this.layers.grid.getBBox().width) {$upper_text.remove();}
            } catch (e) {
                var self = this;
                setTimeout(function(){
                    if ($upper_text.getBBox().x2 > self.layers.grid.getBBox().width) {$upper_text.remove();}
                },0);
            }
    ...
    get_date_info(date, last_date, i) {
        ...
        const date_text = {
            ...
            Hour_upper: date.getDate() !== last_date.getDate()
                        ? date_utils.format(date, 'D MMM')
                        : '',
            'Two Hour_lower': date_utils.format(date, 'HH'),
            'Two Hour_upper': date.getDate() !== last_date.getDate()
                              ? date_utils.format(date, 'D MMM')
                              : '',
        ...
        const x_pos = {
            ...
            'Two Hour_lower': 0,
            'Two Hour_upper': this.options.column_width * 12 / 2,
            'Quarter Day_lower': 0,
    ...
    set_scroll_position() {
        ...
        const scroll_pos =
            ...
            this.startScrollLeft = scroll_pos;
